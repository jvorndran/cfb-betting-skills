...
A skills.sh install provides this skill's instructions only; it does not install the executable.

- Run `kalshi --version` before retrieval. If it is unavailable, tell the user that Node.js 22.12 or newer is required and ask them to run `npm install --global @jvorndran/kalshi-cli`. Do not install it without authorization.
- Do not request an API key or authentication setup. Every endpoint supported by this CLI is public and unauthenticated.
- If installation is unavailable or declined, report the capability gap; do not substitute an authenticated client, trading SDK, or arbitrary HTTP wrapper.

## Workflow

1. Confirm the tool before retrieval.
   - Run `kalshi --version` and the smallest relevant `--help` command.
   - If `kalshi` remains unavailable after the setup guidance, report that limitation.
